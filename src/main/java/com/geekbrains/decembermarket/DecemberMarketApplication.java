...
public class DecemberMarketApplication {
	...
	// использовать уже зарегестрированный номер нельзя)
	// 5. Если пользователь купил товар, то он должен иметь возможность оставить отзыв
	// о товаре, и выставить оценку (in process)
	...
	// отдельную страницу с ифнормацией о заказе (сделано)
	// 2. На этой же странице (из п.1) пользовательн должен указать: адрес
	// доставки, контактный телефон (по-умалочанию показываем телефон (сделано)
	// пользователя)
	// 3. Только после подтверждения на страницу из п.1 заказ должен быть
	// сохранен в базе данных (сделано)
